...
# from llava.constants import DEFAULT_PROTEIN_START_TOKEN, DEFAULT_PROTEIN_END_TOKEN  # ####### change ########
from llava.constants_protein import DEFAULT_PROT_START_TOKEN, DEFAULT_PROT_END_TOKEN  # ####### change ########
...
def load_pretrained_model(model_path, model_base, model_name,
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if os.path.exists(os.path.join(model_path, 'non_lora_trainables.bin')):
                ...
                print('Downloading non-LoRA weights...')
            ...
            # # Ensure that after loading, the layers are still float32
            for layer in model.model.mm_projector:
                if hasattr(layer, 'weight'):
                    layer.weight.data = layer.weight.data.to(torch.float32)
                    print(f"Data type of {layer} weights after enforcing float32: {layer.weight.dtype}")
                if hasattr(layer, 'bias') and layer.bias is not None:
                    layer.bias.data = layer.bias.data.to(torch.float32)
                    print(f"Data type of {layer} bias after enforcing float32: {layer.bias.dtype}")
    ...
    if 'llava' in model_name.lower():
        ...
        # protein_processor = protein_encoder.protein_processor  
        protein_processor = None
    ...
    return tokenizer, model, protein_processor, context_len  
